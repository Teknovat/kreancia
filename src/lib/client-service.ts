...
export class ClientService {
  ...
  async getClients(filters: ClientFilters): Promise<ClientListResponse> {
    ...
    const { search, status, hasOverdue, sortBy, sortOrder, page, limit } = filters;
    ...
    const clientsWithStats: ClientWithStats[] = clients.map((clientData) => {
      const totalCredits = clientData.credits.reduce((sum, credit) => sum + Number(credit.totalAmount), 0);

      const outstandingAmount = clientData.credits.reduce((sum, credit) => sum + Number(credit.remainingAmount), 0);

      const overdueAmount = clientData.credits
        .filter((credit) => credit.dueDate && credit.dueDate < new Date() && Number(credit.remainingAmount) > 0)
        .reduce((sum, credit) => sum + Number(credit.remainingAmount), 0);

      const lastActivity =
        clientData.credits.length > 0
      ...
      return {
        ...
        creditLimit: clientData.creditLimit ? Number(clientData.creditLimit) : undefined,
  ...
  async createClient(data: ClientFormData): Promise<Client> {
    ...
    const createdClient = await client.client.create({
      data: {
        ...data,
        merchantId: this.merchantId,
    ...
    return {
      ...
      address: createdClient.address ?? undefined,
      businessName: createdClient.businessName ?? undefined,
      taxId: createdClient.taxId ?? undefined,
      creditLimit: createdClient.creditLimit ? Number(createdClient.creditLimit) : undefined,
    };
  }
  ...
  async updateClient(clientId: string, data: Partial<ClientFormData>): Promise<Client> {
    ...
    return {
      ...
      businessName: updatedClient.businessName ?? undefined,
      taxId: updatedClient.taxId ?? undefined,
      creditLimit: updatedClient.creditLimit ? Number(updatedClient.creditLimit) : undefined,
    };
  }
  ...
  async getClientSummary(): Promise<ClientSummary> {
    ...
    const [totalClients, avgCreditLimit, totalOutstanding, clientsWithOverdue] = await Promise.all([
      client.client.count({
