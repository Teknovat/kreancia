...
function StatusBadge({ status }: { status: "ACTIVE" | "INACTIVE" | "SUSPENDED" }) {
  ...
  const { label, variant: _variant } = formatClientStatus(status);
...
export default function ClientProfileHeader({
  ...
  const _metrics = calculateClientMetrics(client);
        ...
        <div className="flex flex-col lg:flex-row lg:items-center gap-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="space-y-1">
              ...
              <p className="text-sm text-slate-500">Client depuis le {client.createdAt.toLocaleDateString("fr-FR")}</p>
            </div>
          </div>
          ...
          {/* Quick Actions */}
          <div className="flex items-center gap-2">
            {onEdit && <QuickActionButton icon={Edit} label="Modifier" onClick={onEdit} variant="secondary" />}
            ...
            <ActionsDropdown onEdit={onEdit} onDelete={onDelete} onCall={onCall} onEmail={onEmail} client={client} />
          </div>
        </div>
      </div>
      ...
      <div className="bg-slate-50 px-6 py-4 border-t border-slate-100">
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
          ...
          <MetricCard
            ...
            value={
              client.lastActivity
                ? `${Math.floor((Date.now() - client.lastActivity.getTime()) / (1000 * 60 * 60 * 24))}j`
                : "Aucune"
            }
            icon={Calendar}
            color="slate"
