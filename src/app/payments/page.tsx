                ...
                <div className="flex-1">
                  <div className="flex items-center space-x-4">
                    <div className="w-10 h-10 bg-gray-900 text-white flex items-center justify-center font-bold text-sm">
                      {payment.client?.firstName?.charAt(0) || 'P'}{payment.client?.lastName?.charAt(0) || ''}
                    </div>

                    <div className="space-y-1">
                      <h4 className="font-bold text-gray-900">
                        Paiement #{payment.id.substring(0, 8)} - {payment.client ? `${payment.client.firstName} ${payment.client.lastName}` : 'Client inconnu'}
                      </h4>
                ...
                <div className="flex items-center space-x-8">
                  <div className="text-right">
                    ...
                    <p className="font-bold text-lg text-gray-900">
                      {formatAmount(payment.amountNumber || 0)}
                    </p>
                  </div>

                  <div className="text-right">
                    <p className="text-sm font-medium text-gray-600 uppercase tracking-wide">Alloué</p>
                    <p className={`font-bold text-lg ${payment.isFullyAllocated ? 'text-green-600' : 'text-orange-600'}`}>
                ...
                <div className="border-t border-gray-200 pt-3">
                  ...
                  <div className="space-y-2">
                    {payment.allocations.map((allocation) => (
                      <div key={allocation.id} className="flex items-center justify-between bg-gray-50 px-3 py-2 rounded border">
                        <div className="flex items-center gap-2">
                          <span className="text-sm font-medium text-gray-700">
                            {allocation.credit.label}
                          </span>
                          <span className={`px-2 py-0.5 text-xs rounded font-medium ${
                            allocation.credit.status === 'PAID' ? 'bg-green-100 text-green-700' :
                            allocation.credit.status === 'OVERDUE' ? 'bg-red-100 text-red-700' :
                            'bg-blue-100 text-blue-700'
                          }`}>
                            {allocation.credit.status === 'PAID' ? 'Payé' :
                             allocation.credit.status === 'OVERDUE' ? 'Retard' : 'En cours'}
                          </span>
                        </div>
                        <div className="text-sm font-medium text-gray-900">
                          {formatAmount(allocation.amountNumber || 0)}
                        </div>
                      </div>
                    ))}
                    {!payment.isFullyAllocated && (
                      <div className="flex items-center justify-between bg-orange-50 px-3 py-2 rounded border border-orange-200">
                        ...
                        <div className="text-sm font-medium text-orange-700">
                          {formatAmount((payment.amountNumber || 0) - (payment.totalAllocated || 0))}
                        </div>
                      </div>
                    )}
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-black text-green-900">
                      {loading ? '...' : stats.unallocatedPayments}
                    </p>
                  </div>
