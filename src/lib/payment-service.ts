...
export class PaymentService {
  ...
  private transformPaymentWithDetails(paymentData: any): PaymentWithDetails {
    ...
    const totalAllocated = paymentData.paymentAllocations?.reduce(
      (sum: number, allocation: any) => sum + Number(allocation.amount),
      0
    ) || 0
    const unallocatedAmount = amountNumber - totalAllocated
  ...
  private async validateManualAllocation(
    ...
    for (const allocation of manualAllocations) {
      ...
      if (allocation.amount > credit.remainingAmountNumber) {
        errors.push(`Allocation amount (${allocation.amount}) exceeds remaining amount (${credit.remainingAmountNumber}) for credit ${credit.label}`)
      }
    }
  ...
  async createPayment(data: PaymentFormData): Promise<PaymentWithDetails> {
    ...
    let payment: Payment
    let allocationResult: FIFOAllocationResult
    ...
    await withSecureTransaction(async (tx) => {
      ...
      if (data.allocationMode === 'MANUAL' && data.manualAllocations) {
        allocationResult = await this.allocatePaymentManual(
          payment.id,
          payment.amount,
          data.clientId,
  ...
  async getPayments(filters: PaymentFilters): Promise<PaymentListResponse> {
    ...
    // Transform payments
    const paymentsWithDetails = payments.map(paymentData =>
      this.transformPaymentWithDetails(paymentData)
    )
  ...
  async getPaymentSummary(): Promise<PaymentSummary> {
    ...
    const paymentsByMethodRecord = paymentsByMethod.reduce((acc, item) => {
      acc[item.method] = {
        count: item._count,
