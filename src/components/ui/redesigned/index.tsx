...
export function Heading({ level = 1, variant = "title", className, children, ...props }: HeadingProps) {
  ...
  if (level === 1) return <h1 {...commonProps} />;
  if (level === 2) return <h2 {...commonProps} />;
  if (level === 3) return <h3 {...commonProps} />;
  ...
  return (
    <Card variant="default" className={variants[variant]}>
      <div className="space-y-2">
        ...
        {change && <Text className="text-sm text-gray-600">{change}</Text>}
      </div>
    </Card>
  );
}
